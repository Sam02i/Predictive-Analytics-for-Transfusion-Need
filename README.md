...
**Excluded (label-defining) columns:** `hemoglobin`, `sbp_min`, `heart_rate_max`, `shock_index_mean`, `pulse_pressure_mean`, i.e. columns directly used in, or derived from, the label formula (see [Key Findings](#key-findings)).

</details>

> **Target label:** `needs_transfusion` is a rule-derived synthetic label (`hemoglobin < 7.5 OR (sbp_min < 90 AND heart_rate_max > 100)`), i.e. a demo construct, not a real clinical transfusion-order event.
...
- **Sample size.** 206 patient stays and 14 positive cases is small, i.e. all metrics here are indicative, not definitive.
